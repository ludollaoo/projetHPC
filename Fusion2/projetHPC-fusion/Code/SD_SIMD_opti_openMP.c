...
void SD_SIMD_opti_openMP_2(int nrl, int nrh, int ncl, int nch, int nb_img_max, int N, char* path){
	...
	sprintf(nom, "%s/car_3000.pgm", path);
	...
	for(int n_img = 1; n_img < nb_img_max; n_img++){
		...
		sprintf(nom, "%s/car_3%03d.pgm", path, n_img);
		...
		sprintf(nom, "%s/binaire_car_3%03d.pgm", path, n_img);
